...
const schema = require('./bloodBankGroup.schema');
const { ROLES } = require('../../config/constants');

router.post('/', authenticate, authorize(ROLES.BLOOD_BANK_ADMIN, ROLES.PLATFORM_ADMIN), validate(schema.createGroup), controller.createGroup);
router.get('/', authenticate, controller.listGroups);
router.get('/:id', authenticate, controller.getGroup);
router.post('/:id/members', authenticate, authorize(ROLES.BLOOD_BANK_ADMIN, ROLES.PLATFORM_ADMIN), validate(schema.addMember), controller.addMember);
